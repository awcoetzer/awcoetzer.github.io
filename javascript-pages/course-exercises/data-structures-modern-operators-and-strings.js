...
const flightCode = str => str.slice(0, 3).toUpperCase();

for (const flight of flights.split('+')) {
  const [status, from, to, time] = flight.split(';');
  const output = `${status.includes('Del') ? '⚠️' : ''} ${status.replaceAll('_', ' ').trim()} from [ ${flightCode(from)} ] to [ ${flightCode(to)} ] at ${time.replace(':', 'h')}`;
